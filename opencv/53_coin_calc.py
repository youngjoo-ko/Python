...
# 허프변환 원 검출 함수 사용(리턴값은 검출된 원들의 np.ndarray )
circles = cv2.HoughCircles(blur, cv2.HOUGH_GRADIENT, 1, 30, param1=150, param2=40, minRadius=10, maxRadius=120)
...
if circles is not None:
    for i in range(circles.shape[1]): # 동전 개수만큼(N개)
        ...
        # cv2.circle(dst, (cx,cy), radius, (255,0,255), 2,  cv2.LINE_AA) # 찾은 원에 대해 순서대로 보라색 써클을 그림
